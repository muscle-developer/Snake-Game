...
public class FloatingHUDLevel : MonoBehaviour
{
    ...
    [SerializeField]
    private float levelYOffset = 10f; // Level HUD의 Y축 오프셋, 캐릭터 위로 띄워지는 거리 조정
    ...
    void LateUpdate()
    {
        ...
        {
            // 레벨 텍스트 위치 설정
            Vector3 levelPosition = followTarget.position + new Vector3(0f, levelYOffset, 0f);
            levelText.transform.position = levelPosition;
